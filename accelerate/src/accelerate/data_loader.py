...
class DataLoaderDispatcher(DataLoader, DataLoaderStateMixin):
    ...
    **Available attributes:**
    ...
    def _fetch_batches(self, iterator):
        ...
        def pad_batch_list(batch_list, pad_token_id: int = 2, label_pad_value: int = -100):
            """
            将多个 batch（每个 batch 是一个包含 input_ids/attention_mask/labels 的 dict）拍平成样本级序列，
            分别对 input_ids、attention_mask、labels 做 padding，然后再按原 batch 颗粒度还原。
            优先使用 dataset 中已有的 labels；若缺失则不返回 labels（或可按需回退生成）。
            """
            # 记录每个 batch 含有多少条序列，用于还原
            original_shapes = []
            # 收集样本级序列
            input_ids_list = []
            attn_mask_list = []     # 可能没有，稍后用 input_ids 推导
            labels_list = []        # 可能没有
            ...
            for batch in batch_list:
                ...
                if ids.dim() > 1:
                    # 2D: (B, L)
                    input_ids_list.extend(ids)             # 列表里放 (L,)
                    original_shapes.append(ids.shape[0])   # 该 batch 的样本数
                else:
                    input_ids_list.append(ids)
                    original_shapes.append(1)

                # ---- attention_mask（如果已有就一并收集）----
                if "attention_mask" in batch and batch["attention_mask"] is not None:
                    am = batch["attention_mask"]
                    has_any_attn = True
                    if am.dim() > 1:
                        attn_mask_list.extend(am)
                    else:
                        attn_mask_list.append(am)
                else:
                    # 用占位，后面统一推导
                    attn_mask_list.extend([None] * (original_shapes[-1]))

                # ---- labels（优先使用 dataset 给出的）----
                if "labels" in batch and batch["labels"] is not None:
                    ...
                    # 没有 labels：用占位，稍后决定是否回退或直接不返回 labels
                    labels_list.extend([None] * (original_shapes[-1]))

            # ---- 对 input_ids 进行 padding ----
            batch_input_ids = pad_sequence(input_ids_list, batch_first=True, padding_value=pad_token_id)

            # ---- 处理 attention_mask ----
            if has_any_attn and all(x is not None for x in attn_mask_list):
                # 数据里已有完整的 attention_mask，按 0 pad
                # print("有的兄弟")
                batch_attn_mask = pad_sequence(attn_mask_list, batch_first=True, padding_value=0).long()
            else:
                # 由 input_ids 推导：非 pad_token_id 位置为 1
                batch_attn_mask = (batch_input_ids != pad_token_id).long()

            # ---- 处理 labels ----
            if has_any_labels:
                # 仅对存在的 labels 做 pad，None 的先替换成“空张量”与对应 input_ids 长度一致（全为 ignore_index）
                normed_labels = []
                for i, lb in enumerate(labels_list):
                    if lb is None:
                        # 与对应样本长度一致的“全 ignore”标签
                        normed_labels.append(torch.full_like(input_ids_list[i], fill_value=label_pad_value))
                    else:
                        normed_labels.append(lb)
                batch_labels = pad_sequence(normed_labels, batch_first=True, padding_value=label_pad_value)
            else:
                batch_labels = None
                # 如果你想保留旧行为（用 input_ids 复制 labels），取消下一行注释：
                # batch_labels = batch_input_ids.clone()

            # ---- 按原始 batch 结构还原 ----
            new_batch_list = []
            idx = 0
            for num_slices in original_shapes:
                sl = slice(idx, idx + num_slices)
                new_item = {
                    "input_ids": batch_input_ids[sl],
                    "attention_mask": batch_attn_mask[sl],
                }
                if batch_labels is not None:
                    new_item["labels"] = batch_labels[sl]
                idx += num_slices
                new_batch_list.append(new_item)

            return new_batch_list
        # def pad_batch_list(batch_list):
        #     """
        #     统一所有 batch 的 input_ids 长度，按照最长序列进行填充，并调整 attention_mask。
        #     额外修改：填充后返回原来的 `batch` 结构。
        #     """
        #     original_shapes = []  # 记录每个 batch 里 input_ids 的 shape
        #     input_ids_list = []

        #     # Step 1: 拆分 input_ids，并记录原始 batch 结构
        #     for batch in batch_list:
        #         if batch['input_ids'].dim() > 1:  # 处理 2D 张量
        #             input_ids_list.extend(batch['input_ids'])  # 拆分成 1D 片段
        #             original_shapes.append(batch['input_ids'].shape[0])  # 记录 batch 里有多少个片段
        #         else:
        #             input_ids_list.append(batch['input_ids'])
        #             original_shapes.append(1)  # 只有 1 个序列

        #     # print(f"拆分后 input_ids_list[0] 形状: {input_ids_list[0].shape}")  # 确保拆解成功

        #     # Step 2: 进行 padding
        #     batch_inputs = pad_sequence(input_ids_list, batch_first=True, padding_value=2)

        #     # Step 3: 重新创建 attention_mask
        #     attention_masks = torch.zeros_like(batch_inputs, dtype=torch.long)
        #     attention_masks = attention_masks.masked_fill(batch_inputs != 2, 1)

        #     # Step 4: 还原 batch 结构
        #     new_batch_list = []
        #     idx = 0
        #     for i, batch in enumerate(batch_list):
        #         num_slices = original_shapes[i]  # 该 batch 原本包含的序列数
        #         new_batch_list.append({
        #             'input_ids': batch_inputs[idx:idx + num_slices],  # 取回原 batch 结构
        #             'attention_mask': attention_masks[idx:idx + num_slices],  # 取回原 batch 结构
        #             'labels': batch_inputs[idx:idx + num_slices].clone()
        #         })
        #         idx += num_slices  # 更新索引
        ...
        if self.state.process_index == 0:
            try:
                if self.split_batches:
                    ...
                    batches = pad_batch_list(batches)
                    try:
