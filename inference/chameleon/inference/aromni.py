...
class Options:
    ...
    class Image:
        @dataclass
        class CFG:
            guidance_scale_text: float = 3.07
            guidance_scale_image: float = 1.15
...
class TokenManager:
    ...
    def __init__(
        ...
        self.vocab = VocabInfo(vocab_map)
    ...
    def pil_from_bpe_tokens(self, bpe_tokens: torch.Tensor) -> PIL.Image:
        ...
        if image_tensor.numel() == 0:
            raise ValueError("Empty image token tensor cannot be decoded.")

        if image_tensor.shape[0] < self._IMAGE_TOKEN_COUNT:
            pad_len = self._IMAGE_TOKEN_COUNT - image_tensor.shape[0]
            pad_val = image_tensor[0]
            padding = torch.full(
                (pad_len,),
                int(pad_val.item()) if pad_val.numel() == 1 else int(pad_val),
                dtype=int,
                device=image_tensor.device,
            )
            image_tensor = torch.cat((image_tensor, padding), dim=0).unsqueeze(0)
        else:
            image_tensor = image_tensor[: self._IMAGE_TOKEN_COUNT].unsqueeze(0)
    ...
    def tokenize_image(self, img: Image) -> list[int]:
        img_ids = self.image_tokenizer.img_tokens_from_pil(img)  # [0..8191]
        bpe = self.translation.convert_img2bp2(img_ids).tolist()
        return [self.vocab.begin_image] + bpe + [self.vocab.end_image]
    ...
    #     tokens: list[int] = [self.vocab.bos_id]
    ...
    #         if typ == "text":
    #             tokens += self.tokenize_text(val)

    #         elif typ == "image":
    #             if isinstance(val, str):
    #                 if val.startswith("data:"):
    #                     tokens += self.tokenize_b64img(val.split(",", 1)[1])
    #                 elif val.startswith("file:"):
    #                     tokens += self.tokenize_image(PIL.Image.open(val.split(":", 1)[1]))
    #                 else:
    #                     raise ValueError("Unknown image format.")
    #             elif isinstance(val, Image):
    #                 tokens += self.tokenize_image(val)
    #             else:
...
class TextDecoder(Decoder):
    def __init__(
        ...
        stopping_criteria = [MaxLengthCriteria(max_seq_len)] + [
            StopOnEOSAfterBatchIndex(eos_id, [max_prompt_len] * len(prompt_lens))
        ...
        self.gen = ChameleonGenerator(
            ...
            token_selector=FiniteTransTokenSelector(default_greedy=options.txt.greedy),
        )
        advance(self.gen, max_prompt_len)

    def _allowed_tokens(self) -> list[int]:
        allowed = [self.vocab.eos_id]
        if self.options.txt:
            allowed += self.vocab.text_tokens
        if self.options.img:
            allowed += [self.vocab.begin_image]
        allowed += self.vocab.special_tokens
        return allowed
...
class ImageDecoder(Decoder):
    def __init__(
        ...
        for inp in input_ids:
            if not inp or inp[-1] != self.vocab.begin_image:
                inp.append(self.vocab.begin_image)

        max_prompt_len = max(len(inp) for inp in input_ids) if input_ids else 0
        self.gen = ChameleonGenerator(
            ...
            token_selector=ReplicatedInputTokenSelector(
                (ArgmaxTokenSelector() if options.img.greedy else MultinomialTokenSelector()),
                n=3,
            ),
        )
        advance(self.gen, max_prompt_len)
    ...
    def _split_inputs_for_cfg(self, input_ids: list[list[int]]) -> list[list[int]]:
        allowed = set(self.vocab.image_tokens) | {
            self.vocab.bos_id,
            self.vocab.begin_image,
        ...
        image_conditioned = [
            [tok for tok in sample if tok in allowed] for sample in input_ids
        ]

        unconditioned = [
            [self.vocab.bos_id, self.vocab.begin_image]
        ] * self.batch_size
    ...
    def __next__(self) -> DecodePiece:
        if self.gen_count == 1024:
            ...
            logits = torch.full((self.batch_size, len(self.vocab.all_tokens)), -math.inf)
            logits[:, self.vocab.end_image] = 0
            return DecodePiece(ChameleonGenerator.Token(id=id, logits=logits), TextDecoder)
...
class Generator(Decoder):
    def __init__(
        ...
        if not self.options.txt:
            self.dyngen = DynamicGenerator(ImageDecoder(model, vocab, options, input_ids))
        else:
            self.dyngen = DynamicGenerator(TextDecoder(model, vocab, options, input_ids))
    ...
    def stream(
        ...
        *,
        ...
        if options.txt and options.img and len(batch_input_ids) > 1:
            raise ValueError("Batch generation only supported for one modality at a time.")
