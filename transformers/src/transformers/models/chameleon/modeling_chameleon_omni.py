...
class ChameleonModel(ChameleonPreTrainedModel):
    ...
    def forward(
        ...
        input_ids: torch.LongTensor = None,
        pixel_values: torch.FloatTensor = None,
        audio_ids: Optional[torch.LongTensor] = None,  # 新增参数：audio token ids
        attention_mask: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        past_key_values: Optional[List[torch.FloatTensor]] = None,
        inputs_embeds: Optional[torch.FloatTensor] = None,
        ...
        if inputs_embeds is None:
            ...
            audio_embeds = self.embed_tokens(audio_ids) if audio_ids is not None else text_embeds

            # 计算嵌入的均值
            inputs_embeds = (text_embeds + audio_embeds) / 2
...
class ChameleonForCausalLM(ChameleonPreTrainedModel):
    ...
    #     self,
    #     input_ids: torch.LongTensor = None,
    #     audio_ids: Optional[torch.LongTensor] = None,  # Add audio_ids here
    #     pixel_values: torch.FloatTensor = None,
    #     attention_mask: Optional[torch.Tensor] = None,
    #     position_ids: Optional[torch.LongTensor] = None,
    #     past_key_values: Optional[List[torch.FloatTensor]] = None,
    ...
    #     outputs = self.model(
    #         input_ids=input_ids,
    #         audio_ids=audio_ids,  # Pass the audio_ids to ChameleonModel
    ...
    def forward(
        ...
        inputs_embeds: Optional[torch.FloatTensor] = None,
        labels: Optional[torch.LongTensor] = None,
        audio_labels: Optional[torch.LongTensor] = None,  # Add audio_labels here
        use_cache: Optional[bool] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        ...
        audio_processor =  AllowOnlyTokensLogitsProcessor(self._allowed_aud_tokens())
        # audio_logits = audio_processor(logits.clone())
        audio_logits = logits.clone()
        
        total_loss = None
        label_loss = None
        audio_label_loss = None
        ...
        if audio_labels is not None:
            # Compute loss for `audio_labels` using `audio_logits`
            shift_audio_logits = audio_logits[..., :-1, :].contiguous()
            shift_audio_labels = audio_labels[..., 1:].contiguous()
            shift_audio_logits = shift_audio_logits.view(-1, self.config.vocab_size)
            shift_audio_labels = shift_audio_labels.view(-1)
            shift_audio_labels = shift_audio_labels.to(shift_audio_logits.device)
            audio_label_loss = loss_fct(shift_audio_logits, shift_audio_labels)

        if label_loss is not None or audio_label_loss is not None:
            total_loss = 0
            if label_loss is not None:
                total_loss += label_loss
            if audio_label_loss is not None:
                total_loss += audio_label_loss
    ...
    def _allowed_aud_tokens(self) -> list[int]:
        allowed_aud_tokens = list(range(8724, 12823))  # Exclude 12821
        return allowed_aud_tokens
